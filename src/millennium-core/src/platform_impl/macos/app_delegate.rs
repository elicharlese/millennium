...
pub struct AuxDelegateState {
	/// We store this value in order to be able to defer setting the activation
	/// policy until after the app has finished launching. If the activation
	/// policy is set earlier, the menubar is initially unresponsive on macOS
	/// 10.15 for example.
...
extern "C" fn dealloc(this: &Object, _: Sel) {
	unsafe {
		...
		// As soon as the box is constructed it is immediately dropped, releasing the
		// underlying memory
