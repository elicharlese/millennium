...
static BLOCK_PTR: &str = "millenniumMenuItemBlockPtr";
	...
	pub fn add_native_item(&mut self, item: MenuItem, menu_type: MenuType) -> Option<CustomMenuItem> {
		let menu_details: Option<(Option<MenuId>, *mut Object)> = match item {
			...
			MenuItem::Services => unsafe {
				...
				// we have to assign an empty menu as the app's services menu, macOS will populate it
...
pub(crate) fn make_custom_menu_item(id: MenuId, title: &str, selector: Option<Sel>, accelerators: Option<Accelerator>, menu_type: MenuType) -> *mut Object {
	let title = super::util::strip_mnemonic(title);
...
pub(crate) fn make_menu_item(title: &str, selector: Option<Sel>, accelerator: Option<Accelerator>, menu_type: MenuType) -> *mut Object {
	let alloc = make_menu_alloc();
	unsafe {
		let title = NSString::alloc(nil).init_str(title);
...
fn make_menu_item_class() -> *const Class {
	...
	INIT.call_once(|| unsafe {
		...
		let mut decl = ClassDecl::new("MillenniumMenuItem", superclass).unwrap();
		decl.add_ivar::<usize>(BLOCK_PTR);

		decl.add_method(sel!(dealloc), dealloc_custom_menuitem as extern "C" fn(&Object, _));

		decl.add_method(sel!(fireMenubarAction:), fire_menu_bar_click as extern "C" fn(&Object, _, id));

		decl.add_method(sel!(fireStatusbarAction:), fire_status_bar_click as extern "C" fn(&Object, _, id));
