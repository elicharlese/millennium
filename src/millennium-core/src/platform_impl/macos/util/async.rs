...
// window styles isn't.
pub unsafe fn toggle_full_screen_async(ns_window: id, ns_view: id, not_fullscreen: bool, shared_state: Weak<Mutex<SharedState>>) {
	let ns_window = MainThreadSafe(ns_window);
	let ns_view = MainThreadSafe(ns_view);
...
pub unsafe fn set_maximized_async(ns_window: id, is_zoomed: bool, maximized: bool, shared_state: Weak<Mutex<SharedState>>) {
	let ns_window = MainThreadSafe(ns_window);
	let shared_state = MainThreadSafe(shared_state);
	Queue::main().exec_async(move || {
		if let Some(shared_state) = shared_state.upgrade() {
