...
// 2. translate the coordinate from a bottom-left origin coordinate system to a
// top-left one
...
#[cfg(feature = "tray")]
/// Get the icon Y-axis correctly aligned with Millennium based on the tray icon
/// `NSRect`. Available only with the `tray` feature flag.
pub fn bottom_left_to_top_left_for_tray(rect: NSRect) -> f64 {
	CGDisplay::main().pixels_high() as f64 - rect.origin.y
}

#[cfg(feature = "tray")]
/// Get the cursor Y-axis correctly aligned with Millennium when we click on the
/// tray icon. Available only with the `tray` feature flag.
pub fn bottom_left_to_top_left_for_cursor(point: NSPoint) -> f64 {
	CGDisplay::main().pixels_high() as f64 - point.y
}

/// Converts from Millennium screen-coordinates to macOS screen-coordinates.
/// Millennium: top-left is (0, 0) and y increasing downwards
