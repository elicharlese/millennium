	...
	pub fn build<T: 'static>(self, _window_target: &EventLoopWindowTarget<T>, tray_id: TrayId, tooltip: Option<String>) -> Result<RootSystemTray, OsError> {
		unsafe {
			...
			if let Some(menu) = self.system_tray.tray_menu.clone() {
				...
				// Because setting directly to status bar will overwrite the event callback of
...
/// System tray is a status icon that can show popup menu. It is usually
...
/// Create a `TrayHandler` Class that handle button click event and also menu
/// opening and closing.
///
/// We set the tray menu to tray_target instead of status bar, because setting
/// directly to status bar will overwrite the event callback of the button. When
/// `perform_tray_click` called, it will set the menu to status bar in the end.
/// And when the menu is closed `menu_did_close` will set it to nil again.
fn make_tray_class() -> *const Class {
	...
	INIT.call_once(|| unsafe {
		...
		let mut decl = ClassDecl::new("MillenniumTrayHandler", superclass).unwrap();
		...
		decl.add_method(sel!(menuDidClose:), menu_did_close as extern "C" fn(&mut Object, _, id));
...
extern "C" fn perform_tray_click(this: &mut Object, _: Sel, button: id) {
	unsafe {
		...
		let position: PhysicalPosition<f64> = LogicalPosition::new(frame.origin.x as f64, bottom_left_to_top_left_for_tray(frame)).to_physical(scale_factor);
		...
		let click_type = match event_mask {
			...
			NSEventType::NSLeftMouseDown if key_code.contains(NSEventModifierFlags::NSControlKeyMask) => Some(TrayEvent::RightClick),
		...
		if let Some(click_event) = click_type {
			...
			if *menu != nil {
				...
				if click_event == TrayEvent::RightClick || (*menu_on_left_click && click_event == TrayEvent::LeftClick) {
...
// Set the menu of the status bar to nil, so it won't overwrite the button
// events.
