...
pub fn new_delegate(window: &Arc<UnownedWindow>, initial_fullscreen: bool) -> IdRef {
	...
	unsafe {
		...
		IdRef::new(msg_send![delegate, initWithMillennium: state_ptr])
	}
}
...
extern "C" fn window_did_resign_key(this: &Object, _: Sel, _: id) {
	...
	with_state(this, |state| {
		...
		let view_state: &mut ViewState = unsafe {
			...
			let state_ptr: *mut c_void = *ns_view.get_ivar("millenniumState");
...
extern "C" fn window_will_use_fullscreen_presentation_options(this: &Object, _: Sel, _: id, proposed_options: NSUInteger) -> NSUInteger {
