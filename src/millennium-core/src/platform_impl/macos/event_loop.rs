...
impl<T> EventLoop<T> {
	...
	{
		// This transmute is always safe, in case it was reached through `run`, since
		// our lifetime will be already 'static. In other cases caller should ensure
		// that all data they passed to callback will actually outlive it, some apps
...
#[inline]
pub fn stop_app_on_panic<F: FnOnce() -> R + UnwindSafe, R>(panic_info: Weak<PanicInfo>, f: F) -> Option<R> {
