...
impl<T> EventLoopRunner<T> {
	pub fn catch_unwind<R>(&self, f: impl FnOnce() -> R) -> Option<R> {
		...
		if panic_error.is_none() {
			...
			// inside of `f`. If it was, that error takes priority. If it wasn't, check if
			// our call to catch_unwind caught any panics and set panic_error appropriately.
			match self.panic_error.take() {
				None => match result {
					Ok(r) => Some(r),
					Err(e) => {
...
impl<T> EventLoopRunner<T> {
	...
	unsafe fn dispatch_buffered_events(&self) {
		loop {
			...
			// let` loop the reference returned `borrow_mut()` doesn't get dropped until the
			// end of the loop's body and attempts to add events to the event buffer while
			// in `process_event` will fail.
	...
	/// Attempting to transition *from* `Destroyed` will also reuslt in a panic.
	/// Transitioning to the current state is a no-op. Even if the
	...
	unsafe fn call_new_events(&self, init: bool) {
		let start_cause = match (init, self.control_flow()) {
			...
			(false, ControlFlow::ExitWithCode(_)) | (false, ControlFlow::Wait) => StartCause::WaitCancelled {
