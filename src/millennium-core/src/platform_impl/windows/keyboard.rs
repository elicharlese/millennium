...
lazy_static! {
	pub(crate) static ref KEY_EVENT_BUILDERS: Mutex<HashMap<WindowId, KeyEventBuilder>> = Mutex::new(HashMap::new());
}

/// Stores information required to make `KeyEvent`s.
///
/// A single Millennium `KeyEvent` contains information which the Windows API
/// passes to the application in multiple window messages. In other words: a
/// Millennium `KeyEvent` cannot be built from a single window message.
/// Therefore, this type keeps track of certain information from previous events
/// so that a `KeyEvent` can be constructed when the last event related to a
/// keypress is received.
///
/// `PeekMessage` is sometimes used to determine whether the next window message
/// still belongs to the current keypress. If it doesn't and the current state
/// represents a key event waiting to be dispatched, then said event is
/// considered complete and is dispatched.
///
...
impl KeyEventBuilder {
	...
	pub(crate) fn process_message(&mut self, hwnd: HWND, msg_kind: u32, wparam: WPARAM, lparam: LPARAM, result: &mut ProcResult) -> Vec<MessageAsKeyEvent> {
		match msg_kind {
			win32wm::WM_SETFOCUS => {
			...
			win32wm::WM_KEYDOWN | win32wm::WM_SYSKEYDOWN => {
				...
				*result = ProcResult::Value(LRESULT(0));
				...
				let event_info = PartialKeyEventInfo::from_message(wparam, lparam, ElementState::Pressed, &mut layouts);

				let mut next_msg = MaybeUninit::uninit();
				let peek_retval = unsafe { PeekMessageW(next_msg.as_mut_ptr(), hwnd, WM_KEYFIRST, WM_KEYLAST, PM_NOREMOVE) };
			...
			win32wm::WM_CHAR | win32wm::WM_SYSCHAR => {
				...
				*result = ProcResult::Value(LRESULT(0));
				...
				unsafe {
					let mut next_msg = MaybeUninit::uninit();
					let has_message = PeekMessageW(next_msg.as_mut_ptr(), hwnd, WM_KEYFIRST, WM_KEYLAST, PM_NOREMOVE);
			...
			win32wm::WM_KEYUP | win32wm::WM_SYSKEYUP => {
				*result = ProcResult::Value(LRESULT(0));
				...
				if let Some(event_info) = valid_event_info {
					...
					return vec![MessageAsKeyEvent { event, is_synthetic: false }];
				}
			}
			_ => ()
		}
	...
	fn synthesize_kbd_state(&mut self, key_state: ElementState, kbd_state: &[u8; 256]) -> Vec<MessageAsKeyEvent> {
		...
		if is_key_pressed(VK_CAPITAL) {
			let event = self.create_synthetic(VK_CAPITAL, key_state, caps_lock_on, num_lock_on, locale_id, &mut layouts);
		...
		let do_modifier = |key_events: &mut Vec<_>, layouts: &mut _| {
			const CLEAR_MODIFIER_VKS: [VIRTUAL_KEY; 6] = [VK_LCONTROL, VK_LSHIFT, VK_LMENU, VK_RCONTROL, VK_RSHIFT, VK_RMENU];
...
enum PartialLogicalKey {
	/// Use the text provided by the WM_CHAR messages and report that as a
	/// `Character` variant. If the text consists of multiple grapheme clusters
	/// (user-precieved characters) that means that dead key could not be
	/// combined with the second input, and in that case we should fall back
...
struct PartialKeyEventInfo {
	...
	/// The UTF-16 code units of the text that was produced by the keypress
	/// event. This take all modifiers into account. Including CTRL
	utf16parts: Vec<u16>,
...
impl PartialKeyEventInfo {
	fn from_message(wparam: WPARAM, lparam: LPARAM, state: ElementState, layouts: &mut MutexGuard<'_, LayoutCache>) -> Self {
		...
		// On Windows Ctrl+NumLock = Pause (and apparently Ctrl+Pause -> NumLock). In
		// these cases the KeyCode still stores the real key, so in the name of
		// consistency across platforms, we circumvent this mapping and force the key
		// values to match the keycode. For more on this, read the article by Raymond
		// Chen, titled: "Why does Ctrl+ScrollLock cancel dialogs?"
		...
		let preliminary_logical_key = layout.get_key(mods_without_ctrl, num_lock_on, vkey, scancode, code);
		...
		let logical_key = if let Some(key) = code_as_key.clone() {
			...
			// In some cases we want to use the UNICHAR text for logical_key in order to
			// allow dead keys to have an effect on the character reported by `logical_key`.
...
struct KeyLParam {
	...
	/// This is `previous_state XOR transition_state`. See the lParam for
	/// WM_KEYDOWN and WM_KEYUP for further details.
	pub is_repeat: bool
}
...
/// Gets the keyboard state as reported by messages that have been removed from
/// the event queue. See also: get_async_kbd_state
...
/// Gets the current keyboard state regardless of whether the corresponding
/// keyboard events have been removed from the event queue. See also:
/// get_kbd_state
...
///
/// Due to this equivalence, the system generates a fake Ctrl key-press (and
/// key-release) preceeding every AltGr key-press (and key-release). We check if
/// the current event is a Ctrl event and if the next event is a right Alt
/// (AltGr) event. If this is the case, the current event must be the
/// fake Ctrl event.
...
fn get_location(scancode: ExScancode, hkl: HKL) -> KeyLocation {
	...
	let vkey = VIRTUAL_KEY(unsafe { MapVirtualKeyExW(scancode as u32, MAPVK_VSC_TO_VK_EX, hkl) as u16 });
	...
	match vkey {
		win32km::VK_LSHIFT | win32km::VK_LCONTROL | win32km::VK_LMENU | win32km::VK_LWIN => KeyLocation::Left,
		win32km::VK_RSHIFT | win32km::VK_RCONTROL | win32km::VK_RMENU | win32km::VK_RWIN => KeyLocation::Right,
