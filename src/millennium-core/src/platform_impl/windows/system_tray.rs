...
impl SystemTrayBuilder {
	...
	pub fn build<T: 'static>(self, window_target: &EventLoopWindowTarget<T>, tray_id: TrayId, tooltip: Option<String>) -> Result<RootSystemTray, RootOsError> {
		...
		unsafe {
			...
			SetWindowSubclass(hwnd, Some(tray_subclass_proc), TRAY_SUBCLASS_ID, Box::into_raw(Box::new(traydata)) as _);
...
impl SystemTray {
	...
	pub fn set_menu(&mut self, tray_menu: &Menu) {
		unsafe {
			...
			SendMessageW(self.hwnd, WM_USER_UPDATE_TRAYMENU, WPARAM(tray_menu.hmenu().0 as _), LPARAM(0));
		}
	}
}
...
unsafe extern "system" fn tray_subclass_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM, _id: usize, subclass_input_ptr: usize) -> LRESULT {
	...
	if msg == WM_USER_TRAYICON && matches!(lparam.0 as u32, WM_LBUTTONUP | WM_RBUTTONUP | WM_LBUTTONDBLCLK) {
		...
		let bounds = Rectangle {
			...
			size: PhysicalSize::new((icon_rect.right - icon_rect.left) as f64, (icon_rect.bottom - icon_rect.top) as f64)
		};
