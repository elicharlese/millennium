...
pub fn from_point(x: f64, y: f64) -> Option<MonitorHandle> {
	let hmonitor = unsafe { MonitorFromPoint(POINT { x: x as i32, y: y as i32 }, MONITOR_DEFAULTTONULL) };
...
impl MonitorHandle {
	...
	pub fn size(&self) -> PhysicalSize<u32> {
		let monitor_info = get_monitor_info(self.hmonitor()).unwrap();
		PhysicalSize {
			width: (monitor_info.monitorInfo.rcMonitor.right - monitor_info.monitorInfo.rcMonitor.left) as u32,
			height: (monitor_info.monitorInfo.rcMonitor.bottom - monitor_info.monitorInfo.rcMonitor.top) as u32
		}
	}
