		...
		'main: loop {
			...
			if let Some(wait_until) = wait_until_opt {
				...
				if now < wait_until {
					...
					// compensate for that.
					let resume_reason = MsgWaitForMultipleObjectsEx(None, dur2timeout(wait_until - now).saturating_sub(1), QS_ALLEVENTS, MWMO_INPUTAVAILABLE);
...
unsafe fn public_window_callback_inner<T: 'static>(
	...
	RedrawWindow(subclass_input.event_loop_runner.thread_msg_target(), None, HRGN::default(), RDW_INTERNALPAINT);
		...
		win32wm::WM_NCCALCSIZE => {
			...
			if !win_flags.contains(WindowFlags::DECORATIONS) {
				...
				if util::is_maximized(window) {
					let params = &mut *(lparam.0 as *mut NCCALCSIZE_PARAMS);
					if let Ok(monitor_info) = monitor::get_monitor_info(MonitorFromRect(&params.rgrc[0], MONITOR_DEFAULTTONULL)) {
