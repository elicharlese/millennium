...
use crate::{
	...
	window::{CursorIcon, Fullscreen, Theme, UserAttentionType, WindowAttributes, WindowId as RootWindowId, BORDERLESS_RESIZE_INSET}
};
...
impl Window {
	...
	pub fn set_visible(&self, visible: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::VISIBLE, visible));
		});
	}
	...
	pub fn set_resizable(&self, resizable: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::RESIZABLE, resizable));
		});
	}
	...
	pub fn set_maximized(&self, maximized: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::MAXIMIZED, maximized));
		});
	}
		...
		self.thread_executor.execute_in_thread(move || {
			...
			match (&old_fullscreen, &fullscreen) {
				...
				| (&Some(Fullscreen::Exclusive(_)), &Some(Fullscreen::Exclusive(ref video_mode))) => {
					...
					let mut display_name = OsStr::new(&monitor.inner.native_identifier()).encode_wide().collect::<Vec<_>>();
					...
					let res =
						unsafe { ChangeDisplaySettingsExW(PCWSTR(display_name.as_ptr()), Some(&native_video_mode), HWND::default(), CDS_FULLSCREEN, None) };
				...
				(&Some(Fullscreen::Exclusive(_)), &None) | (&Some(Fullscreen::Exclusive(_)), &Some(Fullscreen::Borderless(_))) => {
					let res = unsafe { ChangeDisplaySettingsExW(PCWSTR::null(), None, HWND::default(), CDS_FULLSCREEN, None) };
			...
			unsafe {
				...
				//
				// Calling `PeekMessageW` here notifies Windows that our process is still
				// running fine, taking control back from the DWM and ensuring that the
				// `SetWindowPos` call below goes through.
	...
	pub fn set_decorations(&self, decorations: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::DECORATIONS, decorations));
		});
	}
	...
	pub fn set_always_on_bottom(&self, always_on_bottom: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::ALWAYS_ON_BOTTOM, always_on_bottom));
		});
	}
	...
	pub fn set_always_on_top(&self, always_on_top: bool) {
		...
		self.thread_executor.execute_in_thread(move || {
			WindowState::set_window_flags(window_state.lock(), window.0, |f| f.set(WindowFlags::ALWAYS_ON_TOP, always_on_top));
		});
	}
	...
	pub fn set_window_icon(&self, window_icon: Option<Icon>) {
		if let Some(ref window_icon) = window_icon {
			window_icon.inner.set_for_window(self.window.0, IconType::Small);
		} else {
	...
	pub fn set_taskbar_icon(&self, taskbar_icon: Option<Icon>) {
		if let Some(ref taskbar_icon) = taskbar_icon {
			taskbar_icon.inner.set_for_window(self.window.0, IconType::Big);
		} else {
	...
	pub fn reset_dead_keys(&self) {
		...
		unsafe {
			...
			ToUnicode(vk, scancode, Some(&kbd_state), mem::transmute(char_buff.as_mut()), 0);
		}
	}
...
impl Drop for Window {
	...
	fn drop(&mut self) {
		...
		unsafe {
			// The window must be destroyed from the same thread that created it, so we send
			// a custom message to be handled by our callback to do the actual work.
...
// Send and Sync are not implemented for HWND and HDC, we have to wrap it and
// implement them manually. For more info see:
...
unsafe fn init<T: 'static>(
	...
	// creating the real window this time, by using the functions in
	// `extra_functions`
	let real_window = {
	...
	if attributes.fullscreen.is_some() {
		...
		let max_size = attributes.max_inner_size.unwrap_or_else(|| PhysicalSize::new(f64::MAX, f64::MAX).into());
	...
	if let Some(window_menu) = attributes.window_menu {
		...
		win.menu = Some(HMenuWrapper(menu::initialize(window_menu, win.hwnd(), menu_handler)));
	}
...
unsafe extern "system" fn window_proc(window: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
	// This window procedure is only needed until the subclass procedure is
	// attached. we need this because we need to respond to WM_NCCALCSIZE as soon as
	// possible in order to make the window borderless if needed.
	match msg {
...
unsafe fn taskbar_mark_fullscreen(handle: HWND, fullscreen: bool) {
	...
	TASKBAR_LIST.with(|task_bar_list_ptr| {
		...
		if task_bar_list.is_none() {
			let result: windows::core::Result<ITaskbarList2> = CoCreateInstance(&TaskbarList, None, CLSCTX_ALL);
		...
		let _ = task_bar_list.unwrap().MarkFullscreenWindow(handle, fullscreen);
	})
}
...
pub(crate) unsafe fn set_skip_taskbar(hwnd: HWND, skip: bool) {
	com_initialized();
	let taskbar_list: ITaskbarList = CoCreateInstance(&TaskbarList, None, CLSCTX_SERVER).expect("failed to create TaskBarList");
	if skip {
