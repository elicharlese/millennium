...
pub(crate) unsafe extern "system" fn subclass_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM, _id: usize, subclass_input_ptr: usize) -> LRESULT {
...
fn execute_edit_command(command: EditCommand) {
	...
	unsafe {
		...
		inputs[0].Anonymous.ki.dwFlags = Default::default();

		inputs[1].r#type = INPUT_KEYBOARD;
		inputs[1].Anonymous.ki.wVk = key;
		inputs[1].Anonymous.ki.dwFlags = Default::default();
