...
impl<T: 'static> EventLoop<T> {
	...
	fn new_gtk() -> Result<EventLoop<T>, Box<dyn Error>> {
		...
		let display = gdk::Display::default().expect("GdkDisplay not found. This usually means `gkt_init` hasn't called yet.");
		...
		window_requests_rx.attach(Some(&context), move |(id, request)| {
			if let Some(window) = app_.window_by_id(id.0) {
				match request {
					...
					WindowRequest::AlwaysOnBottom(always_on_bottom) => window.set_keep_below(always_on_bottom),
					...
					WindowRequest::UserAttention(request_type) => window.set_urgency_hint(request_type.is_some()),
					...
					WindowRequest::WireUpEvents { transparent } => {
						window.add_events(
							EventMask::POINTER_MOTION_MASK
								...
								| EventMask::FOCUS_CHANGE_MASK | EventMask::SCROLL_MASK
						);
						...
						window.connect_button_press_event(|window, event| {
							if !window.is_decorated() && window.is_resizable() && event.button() == 1 {
								if let Some(window) = window.window() {
									...
									// Ignore the `__Unknown` variant so the window receives the click correctly if
									// it is not on the edges.
									match result {
										WindowEdge::__Unknown(_) => (),
										_ => {
											// FIXME: calling `window.begin_resize_drag` uses the default cursor, it should
											// show a resizing cursor instead
								...
								if let Some(window) = window.window() {
									if let Some((cx, cy)) = event.root_coords() {
										if let Some(device) = event.device() {
											...
											match result {
												WindowEdge::__Unknown(_) => (),
												_ => window.begin_resize_drag_for_device(result, &device, 0, cx as i32, cy as i32, event.time())
											}
										}
									}
								}
						...
						window.connect_configure_event(move |window, event| {
							...
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								window_id: RootWindowId(id),
								event: WindowEvent::Moved(LogicalPosition::new(x, y).to_physical(scale_factor as f64))
							}) {
							...
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								window_id: RootWindowId(id),
								event: WindowEvent::Resized(LogicalSize::new(w, h).to_physical(scale_factor as f64))
							}) {
								log::warn!("Failed to send window resized event to event channel: {}", e);
							}
							false
						});
						...
						window.connect_focus_in_event(move |_, _| {
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								...
								log::warn!("Failed to send window focus-in event to event channel: {}", e);
							}
							Inhibit(false)
						});
						...
						window.connect_focus_out_event(move |_, _| {
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								...
								log::warn!("Failed to send window focus-out event to event channel: {}", e);
							}
							Inhibit(false)
						});
						...
						window.connect_enter_notify_event(move |_, _| {
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								...
								log::warn!("Failed to send cursor entered event to event channel: {}", e);
							}
							Inhibit(false)
						});
						...
						window.connect_button_press_event(move |_, event| {
							...
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								...
								log::warn!("Failed to send mouse input preseed event to event channel: {}", e);
							}
							Inhibit(false)
						});
						...
						window.connect_button_release_event(move |_, event| {
							...
							if let Err(e) = tx_clone.send(Event::WindowEvent {
								...
								log::warn!("Failed to send mouse input released event to event channel: {}", e);
							}
							Inhibit(false)
						});
					...
					WindowRequest::Menu(m) => match m {
						...
						(Some(MenuItem::About(name, app)), None) => {
							let mut builder = AboutDialogBuilder::new().program_name(&name).modal(true).resizable(false);
	...
	/// Depends on current control flow and what it should do, an event state is
	/// defined. The whole state flow chart runs like following:
	///
	/// ```ignore
	///                                   Poll/Wait/WaitUntil
	///       +-------------------------------------------------------------------------+
						...
						EventState::DrawQueue => match control_flow {
							...
							_ => {
								if let Ok(id) = draws.try_recv() {
									callback(Event::RedrawRequested(RootWindowId(id)), window_target, &mut control_flow);
								}
