...
use gtk::{prelude::*, AccelFlags, AccelGroup, CheckMenuItem, Menu as GtkMenu, MenuItem as GtkMenuItem, SeparatorMenuItem};
...
impl Menu {
	...
	pub fn add_native_item(&mut self, item: MenuItem, _menu_type: MenuType) -> Option<CustomMenuItem> {
		self.gtk_items.push(GtkMenuInfo {
	...
	pub fn into_gtkmenu(self, tx: &Sender<(WindowId, WindowRequest)>, accel_group: &AccelGroup, window_id: WindowId) -> GtkMenu {
	...
	pub(crate) fn generate_menu<M: gtk::prelude::IsA<gtk::MenuShell>>(
		...
		for menu_item in self.gtk_items {
			let new_item = match menu_item.clone() {
				GtkMenuInfo {
					menu_type: GtkMenuType::Submenu,
					sub_menu: Some(SubmenuDetail { menu, title, enabled, .. }),
					..
				} => {
				...
				GtkMenuInfo {
					...
					let tx_clone = tx.clone();
					menuitem!("Hide", "<Ctrl>H", accel_group, window_id, Some(MenuItem::Hide), tx_clone)
				}
				GtkMenuInfo {
					menu_type: GtkMenuType::Native,
					...
					let tx_clone = tx.clone();
					menuitem!("Close Window", "<Ctrl>W", accel_group, window_id, Some(MenuItem::CloseWindow), tx_clone)
				}
				GtkMenuInfo {
					menu_type: GtkMenuType::Native,
					...
					let tx_clone = tx.clone();
					menuitem!("Quit", "<Ctrl>Q", accel_group, window_id, Some(MenuItem::Quit), tx_clone)
				}
				// TODO add others
				_ => None
			};
