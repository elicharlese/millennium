...
use crate::{
	...
	window::{CursorIcon, Fullscreen, Theme, UserAttentionType, WindowAttributes, BORDERLESS_RESIZE_INSET}
};
...
impl Window {
	pub(crate) fn new<T>(
		...
		// We always create a box and allocate menubar, so if they set_menu after
		// creation we can inject the menubar without re-redendering the whole window
				...
				match preferred_theme {
					...
					Theme::Light => {
						...
						if let Some(theme) = theme_name {
							// Remove dark variant.
							if let Some(theme) = GTK_THEME_SUFFIX_LIST.iter().find(|t| theme.ends_with(*t)).map(|v| theme.strip_suffix(v)) {
		...
		if let Err(e) = window_requests_tx.send((window_id, WindowRequest::WireUpEvents { transparent, cursor_moved })) {
	...
	pub fn inner_position(&self) -> Result<PhysicalPosition<i32>, NotSupportedError> {
		...
		Ok(LogicalPosition::new(x.load(Ordering::Acquire), y.load(Ordering::Acquire)).to_physical(self.scale_factor.load(Ordering::Acquire) as f64))
	}
	...
	pub fn set_outer_position<P: Into<Position>>(&self, position: P) {
		...
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Position((x, y)))) {
	...
	pub fn set_inner_size<S: Into<Size>>(&self, size: S) {
		...
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Size((width, height)))) {
	...
	pub fn set_title(&self, title: &str) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Title(title.to_string()))) {
	...
	pub fn set_menu(&self, menu: Option<menu::Menu>) {
		if let Err(e) = self
			.window_requests_tx
			.send((self.window_id, WindowRequest::SetMenu((menu, self.accel_group.clone(), self.menu_bar.clone()))))
		{
	...
	pub fn set_visible(&self, visible: bool) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Visible(visible))) {
	...
	pub fn set_maximized(&self, maximized: bool) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Maximized(maximized))) {
	...
	pub fn set_fullscreen(&self, fullscreen: Option<Fullscreen>) {
		...
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Fullscreen(fullscreen))) {
	...
	pub fn set_decorations(&self, decorations: bool) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::Decorations(decorations))) {
	...
	pub fn set_always_on_top(&self, always_on_top: bool) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::AlwaysOnTop(always_on_top))) {
			log::warn!("Fail to send always on top request: {}", e);
		}
	}

	pub fn set_window_icon(&self, window_icon: Option<Icon>) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::WindowIcon(window_icon))) {
	...
	pub fn request_user_attention(&self, request_type: Option<UserAttentionType>) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::UserAttention(request_type))) {
	...
	pub fn set_cursor_position<P: Into<Position>>(&self, position: P) -> Result<(), ExternalError> {
		...
		if let Err(e) = self
			.window_requests_tx
			.send((self.window_id, WindowRequest::CursorPosition((x + inner_pos.x, y + inner_pos.y))))
		{
	...
	pub fn current_monitor(&self) -> Option<RootMonitorHandle> {
		...
		// `.window()` returns `None` if the window is invisible; we fallback to the primary monitor
		let monitor = self
			.window
			.window()
	...
	pub fn raw_window_handle(&self) -> RawWindowHandle {
		if self.is_wayland() {
			...
			if let Some(window) = self.window.window() {
				window_handle.surface = unsafe { gdk_wayland_sys::gdk_wayland_window_get_wl_surface(window.as_ptr() as *mut _) };
			}
	...
	pub fn raw_display_handle(&self) -> RawDisplayHandle {
		if self.is_wayland() {
			...
			display_handle.display = unsafe { gdk_wayland_sys::gdk_wayland_display_get_wl_display(self.window.display().as_ptr() as *mut _) };
	...
	pub(crate) fn set_skip_taskbar(&self, skip: bool) {
		if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::SetSkipTaskbar(skip))) {
