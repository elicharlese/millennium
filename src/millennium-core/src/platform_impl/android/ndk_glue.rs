		...
		if let Ok(message) = CHANNEL.1.recv() {
			match message {
				WebViewMessage::CreateWebView(url, mut scripts, devtools) => {
					...
					env.call_method(webview, "addJavascriptInterface", "(Ljava/lang/Object;Ljava/lang/String;)V", &[handler.into(), ipc.into()])?;
...
fn handle_request(env: JNIEnv, request: JObject) -> Result<jobject, jni::errors::Error> {
	...
	let method: JString = env.call_method(request, "getMethod", "()Ljava/lang/String;", &[])?.l()?.into();
	...
	for (header, value) in request_headers.iter()? {
		...
		if let (Ok(header), Ok(value)) = (HeaderName::from_bytes(header.to_bytes()), HeaderValue::from_bytes(value.to_bytes())) {
			headers.insert(header, value);
		}
	}

	if let Some(handler) = REQUEST_HANDLER.get() {
		let response = (handler.0)(WebResourceRequest { url, method, headers });
		if let Some(response) = response {
			let status_code = response.status.as_u16() as i32;
			let reason_phrase = "OK";
			let encoding = "UTF-8";
			let mime_type = if let Some(mime) = response.mimetype { env.new_string(mime)?.into() } else { JObject::null() };
			...
			for (key, value) in response.headers.iter() {
				env.call_method(
					...
					&[
						...
						env.new_string(String::from_utf8_lossy(value.as_bytes()))?.into()
					]
				)?;
			}
			...
			let web_resource_response = env.new_object(
				...
				&[
					...
					status_code.into(),
					env.new_string(reason_phrase)?.into(),
