...
impl<T: 'static> EventLoop<T> {
	...
	where
		F: 'static + FnMut(event::Event<'_, T>, &event_loop::EventLoopWindowTarget<T>, &mut ControlFlow)
	{
		...
		F: FnMut(event::Event<'_, T>, &event_loop::EventLoopWindowTarget<T>, &mut ControlFlow)
	{
		...
		'event_loop: loop {
			...
			match self.first_event.take() {
							...
							if let Some(event) = input_queue.pre_dispatch(event) {
								...
								match &event {
									...
									InputEvent::KeyEvent(key) => {
										...
										// We use the unsafe function directly because we want to forward the keycode
										// value even if it doesn't have a variant defined in the ndk crate
										let keycode_u32 = unsafe { AKeyEvent_getKeyCode(key.ptr().as_ptr()) as u32 };
										let keycode = keycode_u32.try_into().unwrap_or(ndk::event::Keycode::Unknown);
										let physical_key = KeyCode::Unidentified(NativeKeyCode::Android(keycode.into()));
				...
				Some(EventSource::User) => {
					...
					while let Some(event) = user_queue.pop_front() {
						call_event_handler!(event_handler, self.window_target(), control_flow, event::Event::UserEvent(event));
					}
				}
				None => {}
			}

			call_event_handler!(event_handler, self.window_target(), control_flow, event::Event::MainEventsCleared);
			...
			call_event_handler!(event_handler, self.window_target(), control_flow, event::Event::RedrawEventsCleared);

			match control_flow {
				ControlFlow::ExitWithCode(code) => {
					self.first_event = poll(self.looper.poll_once_timeout(Duration::from_millis(0)).unwrap());
				...
				ControlFlow::Poll => {
					self.first_event = poll(self.looper.poll_all_timeout(Duration::from_millis(0)).unwrap());
...
impl MonitorHandle {
	...
	pub fn video_modes(&self) -> impl Iterator<Item = monitor::VideoMode> {
		...
		let mut v = Vec::new();
		// FIXME this is not the real refresh rate (it is guarunteed to support 32 bit
		// color though)
