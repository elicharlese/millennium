...
impl Inner {
	...
	pub fn request_redraw(&self) {
		unsafe {
			if self.gl_or_metal_backed {
				// `setNeedsDisplay` does nothing on UIViews which are directly backed by
				// CAEAGLLayer or CAMetalLayer. Ordinarily the OS sets up a bunch of UIKit state
				// before calling drawRect: on a UIView, but when using raw or gl/metal for
				// drawing this work is completely avoided.
				//
				// The docs for `setNeedsDisplay` don't mention `CAMetalLayer`; however, this
				// has been confirmed via testing.
				//
	...
	pub fn inner_position(&self) -> Result<PhysicalPosition<i32>, NotSupportedError> {
		unsafe {
			...
			return Ok(position.to_physical(scale_factor));
		}
	}
	...
	pub fn inner_size(&self) -> PhysicalSize<u32> {
		unsafe {
			...
			return size.to_physical(scale_factor);
		}
	}
	...
	pub fn set_fullscreen(&self, monitor: Option<Fullscreen>) {
		unsafe {
			let uiscreen = match monitor {
				...
				Some(Fullscreen::Borderless(monitor)) => monitor.unwrap_or_else(|| self.current_monitor_inner()).ui_screen() as id,
				None => {
			...
			// each side
			let () = msg_send![uiscreen, setOverscanCompensation: UIScreenOverscanCompensation::None];
		}
	}
	...
	pub fn fullscreen(&self) -> Option<Fullscreen> {
		unsafe {
			...
			{
				return Some(Fullscreen::Borderless(Some(monitor)));
			} else {
				return None;
			}
		}
	...
	pub fn current_monitor(&self) -> Option<RootMonitorHandle> {
		return Some(self.current_monitor_inner());
	}
	...
	pub fn new<T>(
		...
		unsafe {
			let screen = match window_attributes.fullscreen {
				Some(Fullscreen::Exclusive(ref video_mode)) => video_mode.video_mode.monitor.ui_screen() as id,
			...
			// Like the Windows and macOS backends, we send a `ScaleFactorChanged` and
			// `Resized` event on window creation if the DPI factor != 1.0
...
impl Inner {
	...
	pub fn set_prefers_home_indicator_hidden(&self, hidden: bool) {
		unsafe {
			...
			let () = msg_send![self.view_controller, setPrefersHomeIndicatorAutoHidden: prefers_home_indicator_hidden];
		}
	}
...
impl Inner {
	...
	unsafe fn screen_frame(&self) -> CGRect {
		return self.to_screen_space(msg_send![self.window, bounds]);
	}
	...
	unsafe fn to_screen_space(&self, rect: CGRect) -> CGRect {
		...
		if !screen.is_null() {
			let screen_space: id = msg_send![screen, coordinateSpace];
			return msg_send![self.window, convertRect:rect toCoordinateSpace:screen_space];
		} else {
			return rect;
		}
	}
	...
	unsafe fn from_screen_space(&self, rect: CGRect) -> CGRect {
		...
		if !screen.is_null() {
			let screen_space: id = msg_send![screen, coordinateSpace];
			return msg_send![self.window, convertRect:rect fromCoordinateSpace:screen_space];
		} else {
			return rect;
		}
	}
	...
	unsafe fn safe_area_screen_space(&self) -> CGRect {
		...
		if app_state::os_capabilities().safe_area {
			...
			return self.to_screen_space(safe_bounds);
		} else {
			...
			let status_bar_frame: CGRect = {
				...
				return msg_send![app, statusBarFrame];
			};
...
impl WindowId {
	pub unsafe fn dummy() -> Self {
		return WindowId { window: std::ptr::null_mut() };
	}
}
...
impl From<&mut Object> for WindowId {
	fn from(window: &mut Object) -> WindowId {
		return WindowId { window: window as _ };
	}
}
...
impl Default for PlatformSpecificWindowBuilderAttributes {
	fn default() -> PlatformSpecificWindowBuilderAttributes {
		return PlatformSpecificWindowBuilderAttributes {
