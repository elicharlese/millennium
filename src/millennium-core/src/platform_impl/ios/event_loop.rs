...
impl<T: 'static> EventLoop<T> {
		...
		F: 'static + FnMut(Event<'_, T>, &RootEventLoopWindowTarget<T>, &mut ControlFlow)
	{
		unsafe {
			...
			assert_eq!(
				application,
				ptr::null_mut(),
				"`EventLoop` cannot be `run` after a call to `UIApplicationMain` on iOS. Note: `EventLoop::run` calls `UIApplicationMain` on iOS"
			);
	...
	pub fn create_proxy(&self) -> EventLoopProxy<T> {
		return EventLoopProxy::new(self.window_target.p.sender_to_clone.clone());
	}
...
impl<T> Clone for EventLoopProxy<T> {
	fn clone(&self) -> EventLoopProxy<T> {
		return EventLoopProxy::new(self.sender.clone());
	}
}
...
impl<T> EventLoopProxy<T> {
	fn new(sender: Sender<T>) -> EventLoopProxy<T> {
		unsafe {
			...
			return EventLoopProxy { sender, source };
		}
	}
...
fn setup_control_flow_observers() {
	unsafe {
		// begin is queued with the highest priority to ensure it is processed before
		// other observers
		extern "C" fn control_flow_begin_handler(_: CFRunLoopObserverRef, activity: CFRunLoopActivity, _: *mut c_void) {
			unsafe {
		...
		// Core Animation registers its `CFRunLoopObserver` that performs drawing
		// operations in `CA::Transaction::ensure_implicit` with a priority of
		// `0x1e8480`. We set the main_end priority to be 0, in order to send
		// MainEventsCleared before RedrawRequested. This value was
		// chosen conservatively to guard against apple using different priorities for
		// their redraw observers in different OS's or on different devices. If it so
		// happens that it's too conservative, the main symptom would be non-redraw
		// events coming in after `MainEventsCleared`.
		//
		// The value of `0x1e8480` was determined by inspecting stack traces and the
		// associated registers for every `CFRunLoopAddObserver` call on an iPad Air 2
		// running iOS 11.4.
		//
		// Also tested to be `0x1e8480` on iPhone 8, iOS 13 beta 4.
		extern "C" fn control_flow_main_end_handler(_: CFRunLoopObserverRef, activity: CFRunLoopActivity, _: *mut c_void) {
			unsafe {
				#[allow(non_upper_case_globals)]
				match activity {
		...
		// end is queued with the lowest priority to ensure it is processed after other
		// observers
		extern "C" fn control_flow_end_handler(_: CFRunLoopObserverRef, activity: CFRunLoopActivity, _: *mut c_void) {
			unsafe {
				#[allow(non_upper_case_globals)]
				match activity {
...
impl<F, T: 'static> Debug for EventLoopHandler<F, T> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		return f.debug_struct("EventLoopHandler").field("event_loop", &self.event_loop).finish();
	}
}
