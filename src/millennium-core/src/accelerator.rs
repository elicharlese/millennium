...
impl Accelerator {
	...
	/// Returns `true` if this [`KeyCode`] and [`ModifiersState`] matches this
	/// `Accelerator`.
	///
	/// [`KeyCode`]: KeyCode
	...
	pub fn matches(&self, modifiers: impl Borrow<ModifiersState>, key: impl Borrow<KeyCode>) -> bool {
		...
		let base_mods = ModifiersState::SHIFT | ModifiersState::CONTROL | ModifiersState::ALT | ModifiersState::SUPER;
...
// compatible with Millennium (main) and it also open the option
...
///
/// **This does one thing: it allows specifying accelerators that use the
/// Command key on macOS, but use the Ctrl key on other platforms.**
#[non_exhaustive]
...
///
/// Whenever you receive an event arising from a [GlobalShortcutEvent],
/// [MenuEvent] or [KeyboardInput] , this event contains a `AcceleratorId` which
...
fn test_parse_accelerator() {
	...
	assert_eq!(
		...
		Accelerator {
			...
			mods: ModifiersState::SUPER | ModifiersState::CONTROL | ModifiersState::SHIFT | ModifiersState::ALT,
			key: KeyCode::ArrowUp
		}
	);
	assert_eq!(
