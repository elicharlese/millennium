...
use cocoa::{
	appkit::{
		self, NSApplicationActivationPolicy, NSApplicationActivationPolicyAccessory, NSApplicationActivationPolicyProhibited,
...
/// `with_decorations` method on the base `WindowBuilder`:
///
...
pub trait EventLoopExtMacOS {
	...
	/// [`EventLoopWindowTargetExtMacOS::set_activation_policy_at_runtime`](crate::platform::macos::
...
pub trait EventLoopWindowTargetExtMacOS {
	/// Hide the entire application. In most applications this is typically
	/// triggered with Command-H.
	fn hide_application(&self);

	/// Show the entire application.
	fn show_application(&self);

	/// Hide the other applications. In most applications this is typically
	/// triggered with Command+Option-H.
...
pub trait SystemTrayBuilderExtMacOS {
	...
	///
	/// Images you mark as template images should consist of only black and
	/// clear colors. You can use the alpha channel in the image to adjust the
	/// opacity of black content.
