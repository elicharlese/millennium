...
pub trait EventLoopExtWindows {
	...
	/// # `Window` caveats
	///
	/// Note that any `Window` created on the new thread will be destroyed when
	/// the thread terminates. Attempting to use a `Window` after its parent
	/// thread terminates has unspecified, although explicitly not undefined,
	/// behavior.
	fn new_any_thread() -> Self
	where
		Self: Sized;

	/// By default, this library on Windows will attempt to enable process-wide
	/// DPI awareness. If that's undesirable, you can create an `EventLoop`
	...
	///
	/// The `Window` caveats in
	/// [`new_any_thread`](EventLoopExtWindows::new_any_thread) also apply here.
...
pub trait WindowExtWindows {
	...
	/// If an application has create a modal dialog box by disabling its owner
	/// window (as described in [`WindowBuilderExtWindows::with_owner_window`]),
	/// the application must enable the owner window before destroying the
	/// dialog box. Otherwise, another window will receive the keyboard focus
	/// and be activated.
	///
	/// If a child window is disabled, it is ignored when the system tries to
	/// determine which window should receive mouse messages.
	///
...
pub trait WindowBuilderExtWindows {
	...
	/// Set an owner to the window to be created. Can be used to create a dialog
	/// box, for example. Can be used in combination with
	/// [`WindowExtWindows::set_enable(false)`](WindowExtWindows::set_enable) on
	/// the owner window to create a modal dialog box.
	///
	/// From MSDN:
	...
	///
	/// Parent and menu are mutually exclusive; a child window cannot have a
	/// menu!
	///
	/// The menu must have been manually created beforehand with
	/// [`windows::Win32::UI::WindowsAndMessaging::CreateMenu`] or similar.
	///
	/// Note: Dark mode cannot be supported for win32 menus, it's simply not
	/// possible to change how the menus look. If you use this, it is
	/// recommended that you combine it with `with_theme(Some(Theme::Light))` to
	...
	/// Enables or disables drag and drop support (enabled by default). Will
	/// interfere with other crates that use multi-threaded COM API
	/// (`CoInitializeEx` with `COINIT_MULTITHREADED` instead of
	/// `COINIT_APARTMENTTHREADED`) on the same thread. Note that this library
	/// may still attempt to initialize COM API regardless of this option.
	/// Currently only fullscreen mode does that, but there may be more in the
	/// future. If you need COM API with `COINIT_MULTITHREADED` you must
