...
// This file contains a substantial portion of the UI Events Specification by
// the W3C. In particular, the variant names within `Key` and `KeyCode` and
// their documentation are modified versions of contents of the aforementioned
// specification.
//
...
// license as well as the W3C short notice apply to the `Key` and `KeyCode`
// enums and their variants and the documentation attached to their variants.
...
//
// Copyright © 2021 World Wide Web Consortium, (Massachusetts Institute of
...
use crate::{
	error::OsError,
	platform_impl::{keycode_from_scancode as platform_keycode_from_scancode, keycode_to_scancode as platform_keycode_to_scancode}
};
...
pub enum KeyCode {
	...
	NumLock,
	/// <kbd>0 Ins</kbd> on a keyboard. <kbd>0</kbd> on a phone or remote
	/// control
	Numpad0,
	/// <kbd>1 End</kbd> on a keyboard. <kbd>1</kbd> or <kbd>1 QZ</kbd> on a
	...
	/// control
	Numpad4,
	/// <kbd>5</kbd> on a keyboard. <kbd>5 JKL</kbd> on a phone or remote
	/// control
	Numpad5,
	/// <kbd>6 →</kbd> on a keyboard. <kbd>6 MNO</kbd> on a phone or remote
	/// control
	Numpad6,
	/// <kbd>7 Home</kbd> on a keyboard. <kbd>7 PQRS</kbd> or <kbd>7 PRS</kbd>
	/// on a phone or remote control
	Numpad7,
	/// <kbd>8 ↑</kbd> on a keyboard. <kbd>8 TUV</kbd> on a phone or remote
	/// control
	Numpad8,
	/// <kbd>9 PgUp</kbd> on a keyboard. <kbd>9 WXYZ</kbd> or <kbd>9 WXY</kbd>
	/// on a phone or remote control
	Numpad9,
	/// <kbd>+</kbd>
	NumpadAdd,
	...
	///
	/// This key is typically found below the <kbd>7</kbd> key and to the left
	/// of the <kbd>0</kbd> key.
	///
...
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub enum Key<'a> {
	/// A key string that corresponds to the character typed by the user, taking
	/// into account the user’s current locale setting, and any system-level
	/// keyboard mapping overrides that are in effect.
	Character(&'a str),

	/// This variant is used when the key cannot be translated to any other
	/// variant.
	///
	/// The native scancode is provided (if available) in order to allow the
	/// user to specify keybindings for keys which are not defined by this API.
	Unidentified(NativeKeyCode),
	...
	/// concurrent or subsequent keyboard input. This key value is also used for
	/// the Apple <kbd>Option</kbd> key.
	Alt,
	...
	/// `Shift` key is the level 2 modifier).
	AltGraph,
	...
	///
	/// Used to enable control modifier function for interpreting concurrent or
	/// subsequent keyboard input.
	Control,
	/// The Function switch `Fn` key. Activating this key simultaneously with
	/// another key changes that key’s value to an alternate character or
	/// function. This key is often handled directly in the keyboard hardware
	/// and does not usually generate key events.
	Fn,
	/// The Function-Lock (`FnLock` or `F-Lock`) key. Activating this key
	/// switches the mode of the keyboard to changes some keys' values to an
	/// alternate character or function. This key is often handled directly in
	/// the keyboard hardware and does not usually generate key events.
	FnLock,
	/// The `NumLock` or Number Lock key. Used to toggle numpad mode function
	/// for interpreting subsequent keyboard input.
	NumLock,
	/// Toggle between scrolling and cursor movement modes.
	ScrollLock,
	/// Used to enable shift modifier function for interpreting concurrent or
	/// subsequent keyboard input.
	Shift,
	...
	SymbolLock,
	Hyper,
	/// Used to enable "super" modifier function for interpreting concurrent or
	/// subsequent keyboard input. This key value is used for the "Windows Logo"
	/// key and the Apple `Command` or `⌘` key.
	///
	/// Note: In some contexts (e.g. the Web) this is referred to as the "Meta"
	/// key.
	Super,
	/// The `Enter` or `↵` key. Used to activate current selection or accept
	/// current input. This key value is also used for the `Return` (Macintosh
	/// numpad) key. This key value is also used for the Android
	/// `KEYCODE_DPAD_CENTER`.
	Enter,
	/// The Horizontal Tabulation `Tab` key.
	Tab,
	/// Used in text to insert a space between words. Usually located below the
	/// character keys.
	Space,
	...
	/// (`KEYCODE_MOVE_HOME`). For the mobile phone `Home` key (which goes to
	/// the phone’s main screen), use [`GoHome`].
	///
	/// [`GoHome`]: Self::GoHome
	Home,
	...
	PageUp,
	/// Used to remove the character to the left of the cursor. This key value
	/// is also used for the key labeled `Delete` on MacOS keyboards.
	Backspace,
	...
	Cut,
	/// Used to delete the character to the right of the cursor. This key value
	/// is also used for the key labeled `Delete` on MacOS keyboards when `Fn`
	/// is active.
	Delete,
	...
	/// This key is commonly found between the right `Super` key and the right
	/// `Control` key.
	ContextMenu,
	/// The `Esc` key. This key was originally used to initiate an escape
	/// sequence, but is now more generally used to exit or "escape" the current
	/// context, such as closing a dialog or exiting full screen mode.
	Escape,
	Execute,
	...
	///
	/// Note: Do not use this value for the `Pause` button on media controllers.
	/// Use `"MediaPause"` instead.
	Pause,
	/// Play or resume the current state or application (as appropriate).
	///
	/// Note: Do not use this value for the `Play` button on media controllers.
	/// Use `"MediaPlay"` instead.
	Play,
	...
	/// Note: Note: Some devices might not expose this key to the operating
	/// environment.
	Power,
	...
	/// their code points.
	CodeInput,
	/// The Compose key, also known as "Multi_key" on the X Window System. This
	/// key acts in a manner similar to a dead key, triggering a mode where
	...
	/// Mode key. This key is typically used to switch to a hiragana keyboard
	/// for the purpose of converting input into kanji. (`KEYCODE_KANA`)
	KanjiMode,
	...
	/// with launching "Calculator", and may have a calculator symbol on the
	/// key. (`APPCOMMAND_LAUNCH_APP2`, `KEYCODE_CALCULATOR`)
	LaunchApplication2,
	...
	Lock,
	/// Show a list of media applications: audio/video players and image
	/// viewers. (`VK_APPS`)
	///
	/// Note: Do not confuse this key value with the Windows' `VK_APPS` /
	/// `VK_CONTEXT_MENU` key, which is encoded as `"ContextMenu"`.
	MediaApps,
	...
	VideoModeNext,
	/// Cause device to identify itself in some manner, e.g., audibly or
	/// visibly. (`VK_WINK`)
	Wink,
