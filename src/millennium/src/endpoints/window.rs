...
pub fn into_allowlist_error(variant: &str) -> crate::Error {
	match variant {
		...
		"requestUserAttention" => crate::Error::ApiNotAllowlisted("window > requestUserAttention".to_string()),
		...
		"toggleMaximize" => crate::Error::ApiNotAllowlisted("window > maximize and window > unmaximize".to_string()),
		...
		"setContentProtected" => crate::Error::ApiNotAllowlisted("window > setContentProtected".to_string()),
		...
		"setCursorPosition" => crate::Error::ApiNotAllowlisted("window > setCursorPosition".to_string()),
		...
		"__toggleMaximize" => crate::Error::ApiNotAllowlisted("window > maximize and window > unmaximize".to_string()),
...
impl Cmd {
	...
	async fn manage<R: Runtime>(context: InvokeContext<R>, label: Option<String>, cmd: WindowManagerCmd) -> super::Result<InvokeResponse> {
		Self::_manage(context, label, cmd).await.map_err(crate::error::into_anyhow)
	}

	async fn _manage<R: Runtime>(context: InvokeContext<R>, label: Option<String>, cmd: WindowManagerCmd) -> crate::Result<InvokeResponse> {
		let window = match label {
			Some(l) if !l.is_empty() => context.window.get_window(&l).ok_or(crate::Error::WebviewNotFound)?,
			_ => context.window
		};
		match cmd {
			// Getters
			...
			#[cfg(window_request_user_attention)]
			WindowManagerCmd::RequestUserAttention(request_type) => window.request_user_attention(request_type)?,
			#[cfg(window_set_resizable)]
			...
			#[cfg(window_set_content_protected)]
			WindowManagerCmd::SetContentProtected(protected) => window.set_content_protected(protected)?,
			#[cfg(window_set_size)]
