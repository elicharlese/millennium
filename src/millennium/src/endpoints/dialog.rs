...
pub struct OpenDialogOptions {
	...
	#[serde(default)]
	pub multiple: bool,
	/// Whether the dialog is a directory selection (`true` value) or file
	/// selection (`false` value).
	#[serde(default)]
	pub directory: bool,
...
impl Cmd {
			...
			if options.multiple {
				...
				if let Some(folders) = &folders {
					for folder in folders {
						scopes.allow_directory(folder, options.recursive).map_err(crate::error::into_anyhow)?;
					}
				}
				folders.into()
			} else {
				...
				if let Some(path) = &folder {
					scopes.allow_directory(path, options.recursive).map_err(crate::error::into_anyhow)?;
				}
				folder.into()
			}
	...
	message_dialog!(message_dialog, dialog_message, Option<String>, |label: Option<String>| {
		label
			.map(crate::api::dialog::MessageDialogButtons::OkWithLabel)
			.unwrap_or(crate::api::dialog::MessageDialogButtons::Ok)
	});
	message_dialog!(ask_dialog, dialog_ask, Option<(String, String)>, |labels: Option<(String, String)>| {
		labels
	...
	message_dialog!(confirm_dialog, dialog_confirm, Option<(String, String)>, |labels: Option<(String, String)>| {
		labels
			.map(|(ok, cancel)| crate::api::dialog::MessageDialogButtons::OkCancelWithLabels(ok, cancel))
...
mod tests {
	...
	#[millennium_macros::module_command_test(dialog_open, "dialog > open")]
	#[quickcheck_macros::quickcheck]
	fn open_dialog(_options: OpenDialogOptions) {}

	#[millennium_macros::module_command_test(dialog_save, "dialog > save")]
	#[quickcheck_macros::quickcheck]
