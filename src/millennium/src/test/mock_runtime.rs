...
impl fmt::Debug for RuntimeContext {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("RuntimeContext").field("clipboard", &self.clipboard).finish()
	}
}
...
impl millennium_runtime::GlobalShortcutManager for MockGlobalShortcutManager {
	...
	fn register<F: Fn() + Send + 'static>(&mut self, accelerator: &str, handler: F) -> Result<()> {
		self.context.shortcuts.lock().unwrap().insert(accelerator.into(), Box::new(handler));
		Ok(())
	}
...
impl<T: UserEvent> Dispatch<T> for MockDispatcher {
	...
	fn inner_size(&self) -> Result<PhysicalSize<u32>> {
		Ok(PhysicalSize { width: 0, height: 0 })
	}
