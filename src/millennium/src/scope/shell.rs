...
pub enum ScopeError {
	...
	#[cfg(any(shell_execute, shell_sidecar))]
	#[cfg_attr(doc_cfg, doc(cfg(any(feature = "shell-execute", feature = "shell-sidecar"))))]
	...
	///
	/// Note: This can be called on `shell-execute` feature too due to
	/// [`Scope::prepare`] checking if it's a sidecar from the config.
	#[cfg(any(shell_execute, shell_sidecar))]
	#[cfg_attr(doc_cfg, doc(cfg(any(feature = "shell-execute", feature = "shell-sidecar"))))]
	#[error("The scoped sidecar command was validated, but failed to create the path to the command: {0}")]
	Sidecar(crate::Error),
	...
	#[error("Scoped command argument at position {0} must match regex validation {1} but it was not found")]
	#[cfg(any(shell_execute, shell_sidecar))]
	#[cfg_attr(doc_cfg, doc(cfg(any(feature = "shell-execute", feature = "shell-sidecar"))))]
	...
	/// At least one argument did not pass input validation.
	#[cfg(shell_scope)]
	#[cfg_attr(doc_cfg, doc(cfg(any(feature = "shell-execute", feature = "shell-open"))))]
			...
			(Some(list), ExecuteArgs::List(args)) => list
				...
				.map(|(i, arg)| match arg {
					...
					ScopeAllowedArg::Var { validator } => {
						let value = args.get(i).ok_or_else(|| ScopeError::MissingVar(i, validator.to_string()))?.to_string();
