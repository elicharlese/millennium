...
///
/// This Builder offers a more concise way to construct Millennium plugins than
/// implementing the Plugin trait directly.
///
/// # Conventions
///
/// When using the Builder Pattern it is encouraged to export a function called
/// `init` that constructs and returns the plugin. While plugin authors can
/// provide every possible way to construct a plugin, sticking to the `init`
/// function convention helps users to quickly identify the correct function to
/// call.
///
/// ```rust
/// use millennium::{
...
impl<R: Runtime, C: DeserializeOwned> Builder<R, C> {
	...
	/// It is recommended you use the [millennium::generate_handler] to generate
	/// the input to this method, as the input type is not considered stable
	/// yet.
	///
	/// # Examples
	///
	...
	///
	/// This is a convenience function around [setup_with_config], without the
	/// need to specify a configuration object.
	///
