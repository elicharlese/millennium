...
fn is_symlink<P: AsRef<Path>>(path: P) -> crate::api::Result<bool> {
	// TODO: remove the different implementation once we raise MSRV to at least 1.58
	#[cfg(windows)]
	let ret = symlink_metadata(path).map(|md| md.is_symlink()).map_err(Into::into);

	#[cfg(not(windows))]
	let ret = symlink_metadata(path).map(|md| md.file_type().is_symlink()).map_err(Into::into);
...
pub(crate) fn read_dir_with_options<P: AsRef<Path>>(path: P, recursive: bool, options: ReadDirOptions<'_>) -> crate::api::Result<Vec<DiskEntry>> {
	...
	for entry in fs::read_dir(path)? {
		...
		if let Ok(flag) = is_dir(&path_as_string) {
			files_and_dirs.push(DiskEntry {
				path: path.clone(),
				children: if flag {
					Some(if recursive && (!is_symlink(&path_as_string)? || options.scope.map(|s| s.is_allowed(&path)).unwrap_or(true)) {
