...
impl Client {
	...
	pub async fn send(&self, mut request: HttpRequestBuilder) -> crate::api::Result<Response> {
						...
						if matches!(headers.as_ref().and_then(|h| h.0.get("content-type")).map(|v| v.as_bytes()), Some(b"multipart/form-data")) {
							...
							for (name, part) in &form_body.0 {
								multipart = match part {
									FormPart::File { file, mime, file_name } => {
										// safe to unwrap: always set by previous loop
										let mut file = attohttpc::MultipartFile::new(name, byte_cache.get(name).unwrap());
							...
							return request_builder.body(multipart.build()?).send().map_err(Into::into);
						}
		...
		Ok(Response(request.response_type.unwrap_or(ResponseType::Json), response, request.url))
	}
}
...
impl Client {
	...
	pub async fn send(&self, request: HttpRequestBuilder) -> crate::api::Result<Response> {
		...
		Ok(Response(request.response_type.unwrap_or(ResponseType::Json), response))
	}
}
...
impl HttpRequestBuilder {
		...
		<HeaderValue as TryFrom<V>>::Error: Into<http::Error>
	{
		...
		self.headers.get_or_insert_with(Default::default).0.insert(key?, value?);
		Ok(self)
	}
	...
	/// Sets the type of the response. Interferes with the way we read the
	/// response.
	#[must_use]
...
impl Response {
	...
	/// 	.send(millennium::api::http::HttpRequestBuilder::new("GET", "http://httpbin.org/ip")?)
	/// 	.await?
	/// 	.bytes_stream();
	///
	...
	pub async fn read(self) -> crate::api::Result<ResponseData> {
		...
		for (name, value) in self.1.headers() {
			headers.insert(name.as_str().to_string(), String::from_utf8(value.as_bytes().to_vec())?);
			raw_headers.insert(
