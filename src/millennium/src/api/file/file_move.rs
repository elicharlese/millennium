...
impl<'a> Move<'a> {
	...
	///
	/// The `temp` dir must be explicitly provided since `rename` operations
	/// require files to live on the same filesystem.
	...
	/// Walk in the source and copy all files and create directories if needed
	/// by replacing existing elements. (equivalent to a cp -R)
