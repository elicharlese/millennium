...
impl<R: Read + Seek> Read for ArchiveReader<R> {
	fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
		match self {
			...
			ArchiveReader::GzCompressed(r) => r.read(buf)
		}
	}
}
...
impl<R: Read + Seek> ArchiveReader<R> {
	...
	fn get_mut(&mut self) -> &mut R {
		match self {
			...
			ArchiveReader::GzCompressed(r) => r.get_mut()
		}
	}
}
...
impl<'a, R: Read> Entry<'a, R> {
	...
	/// Extract this entry into `into_path`. If the entry is a directory, the target will be created if it does not
	/// exist. If it's a file, it'll be extracted at this location. Note: You need to include the complete path,
...
impl<'a, R: Read + Seek> Extract<'a, R> {
	...
	pub fn with_files<E: Into<crate::api::Error>, F: FnMut(Entry<'_, &mut ArchiveReader<R>>) -> std::result::Result<bool, E>>(
		&'a mut self,
		mut f: F
	) -> crate::api::Result<()> {
		match self.archive_format {
			...
			ArchiveFormat::Zip => {
				#[cfg(feature = "fs-extract-api")]
				{
					...
					let file_names = archive.file_names().map(|f| f.to_string()).collect::<Vec<String>>();
	...
	/// Extract an entire source archive into a specified path. If the source is
	/// a single compressed file and not an archive, it will be extracted into a
	/// file with the same name inside of `into_dir`.
...
#[cfg(unix)]
fn _set_perms(dst: &Path, f: Option<&mut std::fs::File>, mode: u32, preserve: bool) -> io::Result<()> {
...
#[cfg(windows)]
fn _set_perms(dst: &Path, f: Option<&mut std::fs::File>, mode: u32, _preserve: bool) -> io::Result<()> {
	if mode & 0o200 == 0o200 {
		return Ok(());
	}
	match f {
