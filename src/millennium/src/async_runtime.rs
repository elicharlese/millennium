...
//!
//! Millennium uses [`tokio`] Runtime to initialize code, such as
...
//! here, you could use types in [`tokio`] dierectly. For custom command
...
mod tests {
	...
	async fn handle_abort() {
		...
		let join = handle.spawn(async {
			// Here we sleep 1 second to ensure this task to be uncompleted when abort()
			// invoked.
