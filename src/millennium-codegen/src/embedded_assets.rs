...
///
/// This is the compile time generation of [`millennium_utils::assets::Assets`]
/// from a directory. Assets from the directory are added as compiler
/// dependencies by dummy including the original, uncompressed assets.
///
/// The assets are compressed during this runtime, and can only be represented
/// as a [`TokenStream`] through [`ToTokens`]. The generated code is meant to be
/// injected into an application to include the compressed assets in that
...
pub struct CspHashes {
	...
	/// Inline scripts (`<script>code</script>`). Maps a HTML path to a list of
	/// hashes.
...
impl CspHashes {
	/// Only add a CSP hash to the appropriate category if we think the file
	/// matches
	///
...
impl AssetOptions {
	...
	/// Instruct the asset handler to include a script to freeze the
	/// `Object.prototype` on all HTML files.
	#[must_use]
...
impl EmbeddedAssets {
	/// Compress a collection of files and directories, ready to be generated
	/// into [`Assets`].
	///
	...
	pub fn new(
		...
		mut map: impl FnMut(&AssetKey, &Path, &mut Vec<u8>, &mut CspHashes) -> Result<(), EmbeddedAssetsError>
		...
		let CompressState { assets, csp_hashes } =
			paths
				...
				.try_fold(CompressState { csp_hashes, assets: HashMap::new() }, move |mut state, (prefix, entry)| {
					let (key, asset) = Self::compress_file(&prefix, entry.path(), &mut map, &mut state.csp_hashes)?;
	...
	/// Use highest compression level for release, the fastest one for
	/// everything else
	#[cfg(feature = "compression")]
	...
	fn compress_file(
		prefix: &Path,
		path: &Path,
		map: &mut impl FnMut(&AssetKey, &Path, &mut Vec<u8>, &mut CspHashes) -> Result<(), EmbeddedAssetsError>,
		csp_hashes: &mut CspHashes
	) -> Result<Asset, EmbeddedAssetsError> {
		let mut input = std::fs::read(path).map_err(|error| EmbeddedAssetsError::AssetRead { path: path.to_owned(), error })?;
...
impl ToTokens for EmbeddedAssets {
	fn to_tokens(&self, tokens: &mut TokenStream) {
		...
		tokens.append_all(quote! {{
			use ::millennium::utils::assets::{CspHash, EmbeddedAssets, phf, phf::phf_map};
