...
static OBJC_DRAGGING_ENTERED: Lazy<extern "C" fn(*const Object, Sel, id) -> NSDragOperation> =
	Lazy::new(|| unsafe { std::mem::transmute(method_getImplementation(class_getInstanceMethod(class!(WKWebView), sel!(draggingEntered:)))) });
...
extern "C" fn dragging_entered(this: &mut Object, sel: Sel, drag_info: id) -> NSDragOperation {
	...
	let position = LogicalPosition::<f64>::from((dl.x, frame.size.height - dl.y)).to_physical(scale_factor);

	if !listener.0(&listener.1, FileDropEvent::Hovered { paths, position }) {
		// Reject the Millennium file drop (invoke the OS default behaviour)
