...
impl InnerWebView {
	pub fn new(
		...
		// Use the window hash as the script handler name to prevent from conflict when
		// sharing same web context.
		let window_hash = {
				...
				if let Some(window) = widget.parent() {
					...
					if !window.is_decorated() && window.is_resizable() && !window.is_maximized() {
						if let Some(window) = window.window() {
							...
							// FIXME: calling `window.begin_resize_drag` seems to revert the cursor back to
							// normal style
							window.set_cursor(
								Cursor::from_name(
									&window.display(),
					...
					if let Some(window) = widget.parent() {
						...
						if !window.is_decorated() && window.is_resizable() {
							if let Some(window) = window.window() {
								...
								// we ignore the `__Unknown` variant so the webview receives the click correctly
								// if it is not on the edges.
								match result {
						...
						if let Some(window) = window.window() {
							if let Some((cx, cy)) = event.root_coords() {
								if let Some(device) = event.device() {
									...
									// we ignore the `__Unknown` variant so the window receives the click correctly
									// if it is not on the edges.
									match result {
										WindowEdge::__Unknown(_) => (),
		...
		if attributes.navigation_handler.is_some() || attributes.new_window_handler.is_some() {
			webview.connect_decide_policy(move |_webview, policy_decision, policy_type| {
				let handler = match policy_type {
					...
					PolicyDecisionType::NewWindowAction => &attributes.new_window_handler,
					_ => &None
				};
						...
						if let Some(nav_action) = policy.navigation_action() {
							if let Some(uri_req) = nav_action.request() {
								if let Some(uri) = uri_req.uri() {
									...
									unsafe {
										if allow { webkit_policy_decision_use(pointer) } else { webkit_policy_decision_ignore(pointer) }
									}
								}
							}
						}
