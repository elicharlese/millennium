	...
	pub fn new(data: &WebContextData) -> Self {
		...
		if let Some(data_directory) = data.data_directory() {
			let data_manager = WebsiteDataManagerBuilder::new()
				...
				.indexeddb_directory(&data_directory.join("databases").join("indexeddb").to_string_lossy())
				.build();
			if let Some(cookie_manager) = data_manager.cookie_manager() {
				cookie_manager.set_persistent_storage(&data_directory.join("cookies").to_string_lossy(), CookiePersistentStorage::Text);
			}
		...
		// e.g. Millennium Webview 0.9.4
		let app_info = ApplicationInfo::new();
		app_info.set_name(env!("CARGO_PKG_NAME"));
		app_info.set_version(
			env!("CARGO_PKG_VERSION_MAJOR").parse().expect("invalid Millennium Webview version major"),
			env!("CARGO_PKG_VERSION_MINOR").parse().expect("invalid Millennium Webview version minor"),
...
pub trait WebContextExt {
	...
	///
	/// When duplicate schemes are registered, the duplicate handler will still
	/// be submitted and the `Err(Error::DuplicateCustomProtocol)` will be
	/// returned. It is safe to ignore if you are relying on the platform's
	...
	/// duplicate scheme.
	///
	/// If a duplicate scheme has been passed, its handler will **NOT** be
	/// registered and the function will return
