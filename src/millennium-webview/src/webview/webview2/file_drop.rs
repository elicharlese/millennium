...
// This can be pretty much entirely replaced when WebView2 SDK
// 1.0.721-prerelease becomes stable. https://docs.microsoft.com/en-us/microsoft-edge/webview2/releasenotes#10721-prerelease
...
use windows::Win32::{
	...
	System::{
		...
		Ole::{IDropTarget, IDropTarget_Impl, RegisterDragDrop, RevokeDragDrop, CF_HDROP, DROPEFFECT, DROPEFFECT_COPY, DROPEFFECT_NONE},
...
impl FileDropController {
	...
	fn inject(&mut self, hwnd: HWND, window: Rc<Window>, listener: Rc<dyn Fn(&Window, FileDropEvent) -> bool>) -> bool {
...
impl FileDropHandler {
	pub fn new(window: Rc<Window>, listener: Rc<dyn Fn(&Window, FileDropEvent) -> bool>) -> FileDropHandler {
		Self {
			window,
			listener,
	...
	unsafe fn collect_paths(data_obj: &Option<IDataObject>, paths: &mut Vec<PathBuf>) -> Option<HDROP> {
...
impl IDropTarget_Impl for FileDropHandler {
	...
	fn Drop(&self, pDataObj: &Option<IDataObject>, _grfKeyState: MODIFIERKEYS_FLAGS, pt: &POINTL, _pdwEffect: *mut DROPEFFECT) -> windows::core::Result<()> {
		let mut paths = Vec::new();
		unsafe {
			let hdrop = Self::collect_paths(pDataObj, &mut paths);
