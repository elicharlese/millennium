...
impl InnerWebView {
	...
	fn init_webview(
		...
		// because all of the event handlers are registered for the life of the webview,
		// but if we wanted to be able to remove them later we would hold onto them in
		// self.
		...
		unsafe {
			let handler: ICoreWebView2WindowCloseRequestedEventHandler =
				WindowCloseRequestedEventHandler::create(Box::new(
			...
			controller.SetBounds(rect).map_err(webview2_com::Error::WindowsError)?;
		}
		...
		Self::add_script_to_execute_on_document_created(
			...
			String::from(
				...
				window.addEventListener('mousedown', e => {
					if (e.buttons === 1)
						window.chrome.webview.postMessage('__WEBVIEW_LEFT_MOUSE_DOWN__');
				});
					...
					if let Some(args) = args {
						...
						if js == "__WEBVIEW_LEFT_MOUSE_DOWN__" || js == "__WEBVIEW_MOUSE_MOVE__" {
							if !window.is_decorated() && window.is_resizable() && !window.is_maximized() {
								...
								// don't use `CursorIcon::Arrow` variant or cursor manipulation using css will
								// cause cursor flickering
								if cursor != CursorIcon::Arrow {
									window.set_cursor_icon(cursor);
								}

								if js == "__WEBVIEW_LEFT_MOUSE_DOWN__" {
									// we ignore `HTCLIENT` variant so the webview receives the click correctly if
									// it is not on the edges and prevent conflict with
									// `millennium_core::window::drag_window`.
...
fn load_url_with_headers(webview: &ICoreWebView2, env: &ICoreWebView2Environment, url: &str, headers: http::HeaderMap) {
	let url = encode_wide(url);
