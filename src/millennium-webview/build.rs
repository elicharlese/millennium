...
fn main() {
	...
	if is_android {
		...
		fn env_var(var: &str) -> String {
			std::env::var(var).unwrap_or_else(|_| panic!(" `{}` is not set, which is needed to generate the kotlin files for android.", var))
		}
		...
		let kotlin_out_dir = env_var("MILLENNIUM_ANDROID_KOTLIN_FILES_OUT_DIR");

		let kotlin_out_dir = PathBuf::from(kotlin_out_dir).canonicalize().expect("Failed to canonicalize path");
		let kotlin_files =
			fs::read_dir(PathBuf::from(env_var("CARGO_MANIFEST_DIR")).join("src/webview/android/kotlin")).expect("failed to read kotlin directory");
