...
pub(crate) fn generate_command_enum(mut input: DeriveInput) -> TokenStream {
	...
	match &mut input.data {
		...
		_ => {
			return Error::new(Span::call_site(), "`command_enum` is only implemented for enums")
				.to_compile_error()
				.into();
		}
	};
	...
	match data {
		Data::Enum(data_enum) => {
			...
			for variant in &data_enum.variants {
				...
				let maybe_feature_check = if let Some(f) = feature { quote!(#[cfg(#f)]) } else { quote!() };
				...
				let mut variant_execute_function_name = format_ident!("{}", variant_name.to_string().to_snake_case().to_lowercase());
...
impl Parse for HandlerTestAttributes {
	fn parse(input: ParseStream) -> syn::Result<Self> {
		...
		let allowlist_check_kind = if let (Ok(_), Ok(i)) = (input.parse::<Token![,]>(), input.parse::<Ident>()) {
			if i == "runtime" { AllowlistCheckKind::Runtime } else { AllowlistCheckKind::Serde }
		} else {
