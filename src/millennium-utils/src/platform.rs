...
/// Retrieves the currently running binary's path, taking into account security
/// considerations.
///
/// The path is cached as soon as possible (before even `main` runs) and that
/// value is returned repeatedly instead of fetching the path every time. It is
/// possible for the path to not be found, or explicitly disabled (see following
...
/// # Security
///
/// If the above platform-specific behavior does **not** take place, this
/// function uses the following resolution.
///
/// We canonicalize the path we received from [`std::env::current_exe`] to
/// resolve any soft links. This avoids the usual issue of needing the file to
/// exist at the passed path because a valid current executable result for our
/// purpose should always exist. Notably, [`std::env::current_exe`] also has a
/// security section that goes over a theoretical attack using hard links. Let's
/// cover some specific topics that relate to different ways an attacker might
...
/// original path, including nested symbolic links (`link2 -> link1 -> bin`). On
/// macOS, any results that include a symlink are rejected by default due to
/// lesser symlink protections. This can be disabled, **although discouraged**,
/// with the `process-relaunch-dangerous-allow-symlink-macos` feature.
///
/// ## Hard Links
///
...
/// `/target/(debug|release)/`, the path is
...
pub fn resource_dir(package_info: &PackageInfo, env: &Env) -> crate::Result<PathBuf> {
	...
	#[cfg(target_os = "linux")]
	{
		res = if curr_dir.ends_with("/data/usr/bin") {
			...
			Ok(PathBuf::from(format!("{}/usr/lib/{}", appdir.display(), package_info.package_name())))
		} else {
			// running bundle
			Ok(PathBuf::from(format!("/usr/lib/{}", package_info.package_name())))
		};
	}

	#[cfg(target_os = "macos")]
	{
		res = exe_dir.join("../Resources").canonicalize().map_err(Into::into);
	}
...
mod windows_platform {
	...
	/// Returns the OS version as a tuple like (major, minor, build).
