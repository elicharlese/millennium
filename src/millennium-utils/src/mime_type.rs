...
impl MimeType {
	...
	pub fn parse_from_uri(uri: &str) -> MimeType {
		...
		match suffix {
			...
			Some("svg") => Self::Svg,
			Some("mp4") => Self::Mp4,
			// Assume HTML when a TLD is found for eg. `millennium:://pyke.io` | `millennium://example.com`
			Some(_) => Self::Html,
...
mod tests {
	...
	fn should_parse_mimetype_from_uri() {
		let css = MimeType::parse_from_uri("https://unpkg.com/browse/bootstrap@4.1.0/dist/css/bootstrap-grid.css").to_string();
		...
		let ico: String = MimeType::parse_from_uri("https://icons.duckduckgo.com/ip3/microsoft.com.ico").to_string();
		assert_eq!(ico, String::from("image/vnd.microsoft.icon"));

		let html: String = MimeType::parse_from_uri("https://pyke.io/index.html").to_string();
		assert_eq!(html, String::from("text/html"));

		let js: String = MimeType::parse_from_uri("https://unpkg.com/react@17.0.1/umd/react.production.min.js").to_string();
		assert_eq!(js, "text/javascript".to_string());

		let json: String = MimeType::parse_from_uri("https://unpkg.com/browse/react@17.0.1/build-info.json").to_string();
		...
		let custom_scheme = MimeType::parse_from_uri("millennium://pyke.io").to_string();
