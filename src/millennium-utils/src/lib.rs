...
impl PackageInfo {
	...
	/// On macOS and Windows it's the `name` field, and on Linux it's the `name`
	/// in `kebab-case`.
...
pub enum TitleBarStyle {
	/// A normal title bar.
	Visible,
	/// (macOS) Makes the title bar transparent, so the window background color is shown instead.
	///
	/// Useful if you don't need to have actual HTML under the title bar. This lets you avoid the caveats of using
	/// `TitleBarStyle::Overlay`.
	Transparent,
	/// (macOS) Shows the title bar as a transparent overlay over the window's content.
	///
	/// Keep in mind:
	/// - The height of the title bar is different on different OS versions, which can lead to window the controls and
	///   title not being where you don't expect.
	/// - You need to define a custom drag region to make your window draggable, however due to a limitation you can't
	///   drag the window when it's not in focus.
	/// - The color of the window title depends on the system theme.
	Overlay,
	/// (Windows) Hide the titlebar without disabling window decorations.
	Hidden
}
...
/// The result type of `millennium-utils`.
pub type Result<T> = std::result::Result<T, Error>;

/// The error type of `millennium-utils`.
