...
//! The module to process HTML for Millennium.
...
pub const CSP_TOKEN: &str = "__MILLENNIUM_CSP__";
/// The token used for script nonces.
pub const SCRIPT_NONCE_TOKEN: &str = "__MILLENNIUM_SCRIPT_NONCE__";
/// The token used for style nonces.
pub const STYLE_NONCE_TOKEN: &str = "__MILLENNIUM_STYLE_NONCE__";
		...
		(ref scope, NodeData::Element(element)) => {
			if *scope == TraversalScope::IncludeNode {
				...
				let attrs = attrs
					.map
					.iter()
					.map(|(name, attr)| (QualName::new(attr.prefix.clone(), name.ns.clone(), name.local.clone()), &attr.value))
					.collect::<Vec<_>>();

				serializer.start_elem(element.name.clone(), attrs.iter().map(|&(ref name, value)| (name, &**value)))?
			}
...
pub fn inject_nonce_token(document: &mut NodeRef, dangerous_disable_asset_csp_modification: &DisabledCspModificationKind) {
