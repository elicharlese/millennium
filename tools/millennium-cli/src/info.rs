...
pub(crate) fn cli_upstream_version() -> Result<String> {
		...
		.call()
	{
		...
		Err(ureq::Error::Transport(transport)) => {
			let message = format!("Unable to find updates at the moment. Error: {:?}", transport.kind());
			return Err(anyhow::Error::msg(message));
		}
	};
...
fn npm_latest_version(pm: &PackageManager, name: &str) -> crate::Result<Option<String>> {
	match pm {
		...
		PackageManager::Berry => {
			...
			let output = cmd.arg("npm").arg("info").arg(name).args(["--fields", "version", "--json"]).output()?;
			if output.status.success() {
				let info: crate::PackageJson = serde_json::from_reader(std::io::Cursor::new(output.stdout)).unwrap();
				Ok(info.version)
			} else {
				Ok(None)
			}
...
fn npm_package_version<P: AsRef<Path>>(pm: &PackageManager, name: &str, app_dir: P) -> crate::Result<Option<String>> {
	let (output, regex) = match pm {
		...
		PackageManager::Berry => (
			cross_command("yarn").arg("info").arg(name).current_dir(app_dir).output()?,
...
fn crate_version(millennium_dir: &Path, manifest: Option<&CargoManifest>, lock: Option<&CargoLock>, name: &str) -> (String, Option<String>) {
	...
	let (crate_version_string, found_crate_versions) = match (&manifest, &lock, crate_lock_packages.len()) {
		...
		(None, Some(_lock), 1) => {
			...
			(format!("{version_string} (no manifest)"), vec![crate_lock_package.version.clone()])
		}
		_ => {
...
pub fn command(_options: Options) -> Result<()> {
	...
	let yarn_version = get_version("yarn", &[]).unwrap_or_default().unwrap_or_default();
	...
	if package_manager == PackageManager::Yarn && yarn_version.chars().next().map(|c| c > '1').unwrap_or_default() {
