...
impl Interface for Rust {
	...
	fn new(config: &Config, target: Option<String>) -> crate::Result<Self> {
		let manifest = {
			...
			.unwrap();
			watcher.watcher().watch(&millennium_dir().join("Cargo.toml"), RecursiveMode::Recursive)?;
		...
		if let Some(minimum_system_version) = &config.millennium.bundle.macos.minimum_system_version {
	...
	fn build(&mut self, mut options: Options) -> crate::Result<()> {
		options.features.get_or_insert(Vec::new()).push("custom-protocol".into());

		self::desktop::build(options, &self.app_settings, self.product_name.clone(), &mut self.available_targets, self.config_features.clone())?;
		Ok(())
	}
...
/// Enum that allows for the parsing of `field.workspace = true` in Cargo.toml.
///
/// It allows for values to be inherited from a workspace or defined as needed.
#[derive(Clone, Debug)]
...
impl<'de, T: Deserialize<'de>> serde::de::Deserialize<'de> for MaybeWorkspace<T> {
		...
		D: serde::de::Deserializer<'de>
	{
		...
		if let Ok(workspace) = TomlWorkspaceField::deserialize(serde_value::ValueDeserializer::<D::Error>::new(value.clone())) {
			return Ok(MaybeWorkspace::Workspace(workspace));
		}
		T::deserialize(serde_value::ValueDeserializer::<D::Error>::new(value)).map(MaybeWorkspace::Defined)
	}
}

impl<T> MaybeWorkspace<T> {
	fn resolve(self, label: &str, get_ws_field: impl FnOnce() -> anyhow::Result<T>) -> anyhow::Result<T> {
		match self {
			MaybeWorkspace::Defined(value) => Ok(value),
			MaybeWorkspace::Workspace(TomlWorkspaceField { workspace: true }) => {
				get_ws_field().context(format!("error inheriting `{label}` from workspace root manifest's `workspace.package.{label}`"))
			}
...
impl RustAppSettings {
				...
				cargo_package_settings
					...
					.resolve("version", || {
						get_workspace_version()?
							.context("Workspace Cargo manifest must have the `workspace.package.version` field if a member tries to inherit the version")
					})
	...
	pub fn out_dir(&self, target: Option<String>, debug: bool) -> crate::Result<PathBuf> {
		get_target_dir(target.as_deref().or_else(|| self.cargo_config.build().target()), !debug)
	}
}
...
fn get_cargo_metadata() -> crate::Result<CargoMetadata> {
	let output = Command::new("cargo")
		...
		.current_dir(millennium_dir())
		.output()?;

	if !output.status.success() {
		return Err(anyhow::anyhow!("`cargo metadata` command exited with a non-zero exit code: {}", String::from_utf8(output.stderr)?));
	}
...
fn get_target_dir(target: Option<&str>, is_release: bool) -> crate::Result<PathBuf> {
	let mut path = get_cargo_metadata().with_context(|| "failed to get cargo metadata")?.target_directory;
...
/// Finds the workspace directory via `cargo metadata`.
...
pub fn get_workspace_version() -> crate::Result<Option<String>> {
	// This will already fail because `cargo metadata` fails if there is no version in the workspace root when a package
	// tries to inherit it.
	...
	Ok(toml::from_str::<CargoSettings>(&toml_str)
		.with_context(|| "failed to parse Cargo.toml")?
		.workspace
