...
pub fn run_dev<F: Fn(ExitStatus, ExitReason) + Send + Sync + 'static>(
	...
	let build_child = build_dev_app(options, available_targets, config_features, move |status, reason| {
		if status.success() {
			...
			on_exit(status, if manually_killed_app_.load(Ordering::Relaxed) { ExitReason::TriggeredKill } else { reason });
		}
	})?;
...
pub fn build(
	...
	if options.target == Some("universal-apple-darwin".into()) {
		...
		for triple in ["aarch64-apple-darwin", "x86_64-apple-darwin"] {
			...
			build_production_app(options, available_targets, config_features.clone()).with_context(|| format!("failed to build {triple} binary"))?;
		...
		if !lipo_status.success() {
			return Err(anyhow::anyhow!(format!("Result of `lipo` command was unsuccessful: {lipo_status}. (Is `lipo` installed?)")));
		}
	} else {
...
fn build_dev_app<F: FnOnce(ExitStatus, ExitReason) + Send + 'static>(
	...
	let build_child = match SharedChild::spawn(&mut build_cmd) {
		...
		Err(e) if e.kind() == ErrorKind::NotFound => Err(anyhow::anyhow!(
			...
			runner,
			if runner == "cargo" {
				" Please follow the Millennium prerequisites: https://millennium.pyke.io/docs/main/your-first-app/prerequisites"
			} else {
				""
			}
		)),
...
fn build_production_app(options: Options, available_targets: &mut Option<Vec<Target>>, config_features: Vec<String>) -> crate::Result<()> {
	let mut build_cmd = build_command(options, available_targets, config_features)?;
	let runner = build_cmd.get_program().to_string_lossy().into_owned();
...
fn build_command(options: Options, available_targets: &mut Option<Vec<Target>>, config_features: Vec<String>) -> crate::Result<Command> {
