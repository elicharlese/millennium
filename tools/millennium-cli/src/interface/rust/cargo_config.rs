	...
	pub fn load(path: &Path) -> Result<Self> {
		...
		for current in PathAncestors::new(path) {
			if let Some(path) = get_file_path(&current.join(".cargo"), "config", true)? {
				let contents = fs::read_to_string(&path).with_context(|| format!("failed to read configuration file `{}`", path.display()))?;
				let toml: ConfigSchema = toml::from_str(&contents).with_context(|| format!("could not parse TOML configuration in `{}`", path.display()))?;
...
/// .toml extensions with Cargo's config files, which were historically not used.
...
fn get_file_path(dir: &Path, filename_without_extension: &str, warn: bool) -> Result<Option<PathBuf>> {
	...
	if possible.exists() {
		if warn && possible_with_extension.exists() {
			...
			if !skip_warning {
				log::warn!("Both `{}` and `{}` exist. Using `{}`", possible.display(), possible_with_extension.display(), possible.display());
			}
		}
