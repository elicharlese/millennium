...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
	...
	let package_base_name = format!("{}_{}_{}", settings.main_binary_name(), settings.version_string(), arch);
	...
	let (data_dir, _) = generate_data(settings, &package_dir).with_context(|| "Failed to build data folders and files")?;
	...
	create_archive(vec![debian_binary_path, control_tar_gz_path, data_tar_gz_path], &package_path).with_context(|| "Failed to create package archive")?;
	Ok(vec![package_path])
}

/// Generate the debian data folders and files.
pub fn generate_data(settings: &Settings, package_dir: &Path) -> crate::Result<(PathBuf, BTreeSet<DebIcon>)> {
	// Generate data files.
	...
	for bin in settings.binaries() {
		...
		common::copy_file(&bin_path, &bin_dir.join(bin.name())).with_context(|| format!("Failed to copy binary from {:?}", bin_path))?;
	}
	...
	let icons = generate_icon_files(settings, &data_dir).with_context(|| "Failed to create icon files")?;
...
fn copy_custom_files(settings: &Settings, data_dir: &Path) -> crate::Result<()> {
	for (deb_path, path) in settings.deb().files.iter() {
		let deb_path = if deb_path.is_absolute() { deb_path.strip_prefix("/").unwrap() } else { deb_path };
		if path.is_file() {
