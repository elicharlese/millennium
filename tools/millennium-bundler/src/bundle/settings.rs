...
pub struct UpdaterSettings {
	...
	pub dialog: bool,
	/// Args to pass to `msiexec.exe` when running the updater on Windows.
...
pub struct DebianSettings {
	...
	/// Maps the path on the debian package to the path of the file to include (relative to the current working
	/// directory).
...
pub struct MacOsSettings {
	...
	/// `/Library/Frameworks/`, and `/Network/Library/Frameworks/`), or a path to a specific framework bundle (e.g.
	/// `./data/frameworks/SDL2.framework`).  Note that this setting just makes millennium-bundler copy the specified
	/// frameworks into the OS X app bundle (under `Foobar.app/Contents/Frameworks/`); you are still responsible for:
	///
	/// - arranging for the compiled binary to link against those frameworks (e.g. by emitting lines like
	///   `cargo:rustc-link-lib=framework=SDL2` from your `build.rs` script)
	///
	/// - embedding the correct rpath in your binary (e.g. by running `install_name_tool -add_rpath
	...
	/// `cargo:rustc-env=MACOSX_DEPLOYMENT_TARGET=10.11`.
...
pub struct WindowsSettings {
	/// The file digest algorithm to use for creating file signatures. Required for code signing. SHA-256 is
	/// recommended.
	...
	/// Whether to use Time-Stamp Protocol (TSP, a.k.a. RFC 3161) for the timestamp server. Your code signing provider
	/// may use a TSP timestamp server, like e.g. SSL.com does. If so, enable TSP by setting to true.
	pub tsp: bool,
	/// WiX configuration.
...
pub struct BundleSettings {
	...
	///
	/// Run `millennium build --help` for more info on targets.
	///
...
impl SettingsBuilder {
	...
	///
	/// Bundle settings will be read from from $MILLENNIUM_DIR/.millenniumrc if it exists and fallback to Cargo.toml's
	/// [package.metadata.bundle].
	pub fn build(self) -> crate::Result<Settings> {
		let target = if let Some(t) = self.target { t } else { target_triple()? };

		Ok(Settings {
			package: self.package_settings.expect("package settings is required"),
			package_types: self.package_types,
			project_out_directory: self.project_out_directory.expect("out directory is required"),
			binaries: self.binaries,
			bundle_settings: BundleSettings {
				external_bin: self.bundle_settings.external_bin.as_ref().map(|bins| external_binaries(bins, &target)),
...
impl Settings {
	...
	pub fn copy_resources(&self, path: &Path) -> crate::Result<()> {
		for src in self.resource_files() {
			let src = src?;
			let dest = path.join(millennium_utils::resources::resource_relpath(&src));
			common::copy_file(&src, &dest)?;
		}
		Ok(())
	}
	...
	pub fn short_description(&self) -> &str {
		self.bundle_settings.short_description.as_ref().unwrap_or(&self.package.description)
	}
