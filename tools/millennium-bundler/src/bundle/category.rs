...
mod tests {
	...
	fn category_from_string_ok() {
		...
		assert_eq!(AppCategory::from_str("public.app-category.developer-tools"), Ok(AppCategory::DeveloperTool));
		assert_eq!(AppCategory::from_str("public.app-category.role-playing-games"), Ok(AppCategory::RolePlayingGame));
