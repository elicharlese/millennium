...
pub fn target_triple() -> Result<String, crate::Error> {
	...
	let arch = match arch_res {
		...
		Err(err) => {
			warn!("failed to determine target arch using rustc, error: `{}`. The fallback is the architecture of the machine that compiled this crate.", err);
			if cfg!(target_arch = "x86") {
				...
				return Err(crate::Error::ArchError(String::from("Unable to determine target-architecture")));
			}
		}
	};
	...
	let os = if cfg!(target_os = "linux") {
		...
		return Err(crate::Error::ArchError(String::from("Unable to determine target-os")));
	};
