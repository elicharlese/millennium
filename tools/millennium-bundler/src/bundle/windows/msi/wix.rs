...
fn run_light(wix_toolset_path: &Path, build_path: &Path, arguments: Vec<String>, extensions: &Vec<PathBuf>, output_path: &Path) -> crate::Result<()> {
...
pub fn build_wix_app_installer(settings: &Settings, wix_toolset_path: &Path, updater: bool) -> crate::Result<Vec<PathBuf>> {
	...
	data.insert("webview_installer_args", to_json(if silent_webview_install { "/silent" } else { "" }));
...
fn generate_binaries_data(settings: &Settings) -> crate::Result<Vec<Binary>> {
	...
	for src in settings.external_binaries() {
		...
		binaries.push(Binary {
			...
			id: regex.replace_all(&dest_filename.replace('-', "_"), "").to_string()
		});
	}
	...
	for bin in settings.binaries() {
		if !bin.main() {
			binaries.push(Binary {
				...
				id: regex.replace_all(&bin.name().replace('-', "_"), "").to_string()
			})
		}
	}
