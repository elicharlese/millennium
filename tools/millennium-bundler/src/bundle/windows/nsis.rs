...
use crate::{
	bundle::{
		common::CommandExt,
		windows::util::{
			download, download_and_verify, extract_zip, remove_unc_lossy, try_sign, HashAlgorithm, WEBVIEW2_BOOTSTRAPPER_URL, WEBVIEW2_X64_INSTALLER_GUID,
...
const NSIS_URL: &str = "https://sourceforge.net/projects/nsis/files/NSIS%203/3.08/nsis-3.08.zip/download";
const NSIS_SHA1: &str = "057e83c7d82462ec394af76c87d06733605543d4";
const NSIS_NSCURL_URL: &str = "https://github.com/tauri-apps/binary-releases/releases/download/nsis-plugins-v0/NScurl-1.2022.6.7.zip";
...
const NSIS_NSPROCESS_URL: &str = "https://github.com/tauri-apps/binary-releases/releases/download/nsis-plugins-v0/NsProcess.zip";
const NSIS_SEMVER_COMPARE: &str = "https://github.com/tauri-apps/nsis-semvercompare/releases/download/v0.2.0/nsis_semvercompare.dll";
...
pub fn bundle_project(settings: &Settings, updater: bool) -> crate::Result<Vec<PathBuf>> {
	let millennium_tools_path = dirs_next::cache_dir().unwrap().join("millennium");
	let nsis_toolset_path = millennium_tools_path.join("NSIS");

	if !nsis_toolset_path.exists() {
		get_and_extract_nsis(&nsis_toolset_path, &millennium_tools_path)?;
	} else if NSIS_REQUIRED_FILES.iter().any(|p| !nsis_toolset_path.join(p).exists()) {
		warn!("NSIS directory is missing some files. Recreating it.");
		std::fs::remove_dir_all(&nsis_toolset_path)?;
		get_and_extract_nsis(&nsis_toolset_path, &millennium_tools_path)?;
	}

	build_nsis_app_installer(settings, &nsis_toolset_path, &millennium_tools_path, updater)
}

// Gets NSIS and verifies the download via Sha1
fn get_and_extract_nsis(nsis_toolset_path: &Path, millennium_tools_path: &Path) -> crate::Result<()> {
	info!("Verifying NSIS package");

	let data = download_and_verify(NSIS_URL, NSIS_SHA1, HashAlgorithm::Sha1)?;
	info!("extracting NSIS");
	extract_zip(&data, millennium_tools_path)?;
	rename(millennium_tools_path.join("nsis-3.08"), nsis_toolset_path)?;
	...
	extract_zip(&data, &nsis_plugins)?;
	copy(nsis_plugins.join("ReleaseUnicode").join("ApplicationID.dll"), nsis_plugins.join("x86-unicode").join("ApplicationID.dll"))?;

	let data = download(NSIS_NSPROCESS_URL)?;
	info!("extracting NSIS NsProcess plugin");
	extract_zip(&data, &nsis_plugins)?;
	copy(nsis_plugins.join("Plugin").join("nsProcessW.dll"), nsis_plugins.join("x86-unicode").join("nsProcess.dll"))?;

	let data = download_and_verify(NSIS_SEMVER_COMPARE, NSIS_SEMVER_COMPARE_SHA1, HashAlgorithm::Sha1)?;
	write(nsis_plugins.join("x86-unicode").join("nsis_semvercompare.dll"), data)?;

	Ok(())
}

fn build_nsis_app_installer(settings: &Settings, nsis_toolset_path: &Path, millennium_tools_path: &Path, updater: bool) -> crate::Result<Vec<PathBuf>> {
	...
	if let Some(nsis) = &settings.windows().nsis {
		...
		if let Some(installer_icon) = &nsis.installer_icon {
			data.insert("installer_icon", to_json(remove_unc_lossy(installer_icon.canonicalize()?)));
		}
		if let Some(header_image) = &nsis.header_image {
			data.insert("header_image", to_json(remove_unc_lossy(header_image.canonicalize()?)));
		}
		if let Some(sidebar_image) = &nsis.sidebar_image {
			data.insert("sidebar_image", to_json(remove_unc_lossy(sidebar_image.canonicalize()?)));
		}
	}
	data.insert(
		"install_mode",
	...
	let main_binary = settings
		...
		.find(|bin| bin.main())
		.ok_or_else(|| anyhow::anyhow!("Failed to get main binary"))?;
	data.insert("main_binary_name", to_json(main_binary.name().replace(".exe", "")));
	data.insert("main_binary_path", to_json(settings.binary_path(main_binary)));
	...
	| WebviewInstallMode::OfflineInstaller { silent } = settings.windows().webview_install_mode
	{
		silent
	} else {
		true
	...
	let webview2_installer_args = to_json(if silent_webview2_install { "/silent" } else { "" });
	...
	match webview2_install_mode {
		WebviewInstallMode::EmbedBootstrapper { silent: _ } => {
			let webview2_bootstrapper_path = millennium_tools_path.join("MicrosoftEdgeWebview2Setup.exe");
			std::fs::write(&webview2_bootstrapper_path, download(WEBVIEW2_BOOTSTRAPPER_URL)?)?;
		...
		WebviewInstallMode::OfflineInstaller { silent: _ } => {
			...
			let offline_installer_path = millennium_tools_path.join("Webview2OfflineInstaller").join(guid).join(arch);
	...
	write(&installer_nsi_path, handlebars.render("installer.nsi", &data)?)?;

	let package_base_name = format!("{}_{}_{}-setup", main_binary.name().replace(".exe", ""), settings.version_string(), arch,);
...
fn generate_resource_data(settings: &Settings) -> crate::Result<ResourcesMap> {
	...
	for src in settings.resource_files() {
		...
		resources.insert(resource_path, (target_path.parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default(), target_path));
	}
